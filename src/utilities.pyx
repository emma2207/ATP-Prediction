...
cdef void update_probability_y(
    ...
    # Explicitly update FPE for the corners
    p_now[0, 0] = p_last[0, 0] + dt*(
        -(drift_at_pos[1, 1, 0]*p_last[1, 0]-drift_at_pos[1, N-1, 0]*p_last[N-1, 0])/(2.0*dx)
        +(diffusion_at_pos[2, 1, 0]*p_last[1, 0]-2.0*diffusion_at_pos[2, 0, 0]*p_last[0, 0]+diffusion_at_pos[2, N-1, 0]*p_last[N-1, 0])/(dx*dx)
        +(diffusion_at_pos[3, 0, 1]*p_last[0, 1]-2.0*diffusion_at_pos[3, 0, 0]*p_last[0, 0]+diffusion_at_pos[3, 0, N-1]*p_last[0, N-1])/(dx*dx)
        )
    p_now[0, N-1] = p_last[0, N-1] + dt*(
        -(drift_at_pos[1, 1, N-1]*p_last[1, N-1]-drift_at_pos[1, N-1, N-1]*p_last[N-1, N-1])/(2.0*dx)
        +(diffusion_at_pos[2, 1, N-1]*p_last[1, N-1]-2.0*diffusion_at_pos[2, 0, N-1]*p_last[0, N-1]+diffusion_at_pos[2, N-1, N-1]*p_last[N-1, N-1])/(dx*dx)
        +(diffusion_at_pos[3, 0, 0]*p_last[0, 0]-2.0*diffusion_at_pos[3, 0, N-1]*p_last[0, N-1]+diffusion_at_pos[3, 0, N-2]*p_last[0, N-2])/(dx*dx)
        )
    p_now[N-1, 0] = p_last[N-1, 0] + dt*(
        -(drift_at_pos[1, 0, 0]*p_last[0, 0]-drift_at_pos[1, N-2, 0]*p_last[N-2, 0])/(2.0*dx)
        +(diffusion_at_pos[2, 0, 0]*p_last[0, 0]-2.0*diffusion_at_pos[2, N-1, 0]*p_last[N-1, 0]+diffusion_at_pos[2, N-2, 0]*p_last[N-2, 0])/(dx*dx)
    ...
    for i in range(1, N-1):
        ...
        # Explicitly update FPE for edges not corners
        p_now[0, i] = p_last[0, i] + dt*(
            -(drift_at_pos[1, 1, i]*p_last[1, i]-drift_at_pos[1, N-1, i]*p_last[N-1, i])/(2.0*dx)
            +(diffusion_at_pos[2, 1, i]*p_last[1, i]-2.0*diffusion_at_pos[2, 0, i]*p_last[0, i]+diffusion_at_pos[2, N-1, i]*p_last[N-1, i])/(dx*dx)
            +(diffusion_at_pos[3, 0, i+1]*p_last[0, i+1]-2.0*diffusion_at_pos[3, 1, i]*p_last[0, i]+diffusion_at_pos[3, 0, i-1]*p_last[0, i-1])/(dx*dx)
            )
        p_now[i, 0] = p_last[i, 0] + dt*(
            - (drift_at_pos[1, i+1, 0]*p_last[i+1, 0]-drift_at_pos[1, i-1, 0]*p_last[i-1, 0])/(2.0*dx)
            + (diffusion_at_pos[2, i+1, 0]*p_last[i+1, 0]-2.0*diffusion_at_pos[2, i, 0]*p_last[i, 0]+diffusion_at_pos[2, i-1, 0]*p_last[i-1, 0])/(dx*dx)
        ...
        for j in range(1, N-1):
            p_now[i, j] = p_last[i, j] + dt*(
                -(drift_at_pos[1, i+1, j]*p_last[i+1, j]-drift_at_pos[1, i-1, j]*p_last[i-1, j])/(2.0*dx)
                +(diffusion_at_pos[2, i+1, j]*p_last[i+1, j]-2.0*diffusion_at_pos[2, i, j]*p_last[i, j]+diffusion_at_pos[2, i-1, j]*p_last[i-1, j])/(dx*dx)
                +(diffusion_at_pos[3, i, j+1]*p_last[i, j+1]-2.0*diffusion_at_pos[3, i, j]*p_last[i, j]+diffusion_at_pos[3, i, j-1]*p_last[i, j-1])/(dx*dx)
                )

        # Explicitly update FPE for rest of edges not corners
        p_now[N-1, i] = p_last[N-1, i] + dt*(
            -(drift_at_pos[1, 0, i]*p_last[0, i]-drift_at_pos[1, N-2, i]*p_last[N-2, i])/(2.0*dx)
            +(diffusion_at_pos[2, 0, i]*p_last[0, i]-2.0*diffusion_at_pos[2, N-1, i]*p_last[N-1, i]+diffusion_at_pos[2, N-2, i]*p_last[N-2, i])/(dx*dx)
        ...
        p_now[i, N-1] = p_last[i, N-1] + dt*(
            -(drift_at_pos[1, i+1, N-1]*p_last[i+1, N-1]-drift_at_pos[1, i-1, N-1]*p_last[i-1, N-1])/(2.0*dx)
            +(diffusion_at_pos[2, i+1, N-1]*p_last[i+1, N-1]-2.0*diffusion_at_pos[2, i, N-1]*p_last[i, N-1]+diffusion_at_pos[2, i-1, N-1]*p_last[i-1, N-1])/(dx*dx)
